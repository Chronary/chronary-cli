...
func TestWebhooksListCommand(t *testing.T) {
	...
	rootCmd.SetArgs([]string{"webhooks", "list", "--api-key", "chr_sk_live_test", "--base-url", srv.URL, "--output", "json"})
...
func TestWebhooksGetCommand(t *testing.T) {
	...
	rootCmd := NewRootCmd("test")
	rootCmd.SetArgs([]string{"webhooks", "get", "whk_1", "--api-key", "chr_sk_live_test", "--base-url", srv.URL, "--output", "json"})
	require.NoError(t, rootCmd.Execute())
}
...
func TestWebhooksUpdateCommand(t *testing.T) {
	...
	rootCmd := NewRootCmd("test")
	rootCmd.SetArgs([]string{"webhooks", "update", "whk_1", "--active=false", "--api-key", "chr_sk_live_test", "--base-url", srv.URL, "--output", "json"})
	require.NoError(t, rootCmd.Execute())
}
...
func TestWebhooksDeleteCommand(t *testing.T) {
	...
	rootCmd := NewRootCmd("test")
	rootCmd.SetArgs([]string{"webhooks", "delete", "whk_1", "--force", "--api-key", "chr_sk_live_test", "--base-url", srv.URL})
	require.NoError(t, rootCmd.Execute())
}
